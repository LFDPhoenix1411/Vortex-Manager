const { getAllUsers, updateUser, deleteUser } = require("../scripts/SQL.js");

// Update Function // DEPRECATED
async function updateData(ID) {
  console.group(`Changed User - ID ${ID}`);
  console.log("Changed :", {
    BALANCE: null,
    RANK: null,
    IMAGE_URL: null,
...
async function fetchUsers() {
  ...
  for (let i = 0; i < userCount; i++) {
    const user = users[i];
    console.info(user);
    addCard(user.ID, user.NAME, user.BALANCE, user.RANK, user.IMAGE_URL);
  }
  // Update Title Bar
